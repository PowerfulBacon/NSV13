...
/datum/crewmonitor/proc/update_data(z)
	...
	for(var/mob/living/carbon/human/H in GLOB.carbon_list)
		...
		if ((H.z == 0 || H.get_z_level(TRUE) == get_z_level(TRUE) || (is_station_level(H.z) && is_station_level(z))) && (istype(H.w_uniform, /obj/item/clothing/under) || nanite_sensors))
			U = H.w_uniform
			...
			if (nanite_sensors || ((U.has_sensor > 0) && U.sensor_mode))
				...
				if (H.z == 0 && (!pos || (pos.get_z_level(TRUE) != get_z_level(TRUE)) && !(is_station_level(pos.z) && is_station_level(z))))
					continue

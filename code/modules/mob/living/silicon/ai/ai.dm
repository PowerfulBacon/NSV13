...
/mob/living/silicon/ai/can_interact_with(atom/A)
	...
	if ((ai.get_z_level(TRUE) != target.get_z_level(TRUE)) && !is_station_level(ai.z))
		return FALSE
...
/mob/living/silicon/ai/proc/botcall()
	...
	var/ai_Zlevel = ai_current_turf.get_z_level(TRUE)
	var/d
	d += "<A HREF=?src=[REF(src)];botrefresh=1>Query network status</A><br>"
	d += "<table width='100%'><tr><td width='40%'><h3>Name</h3></td><td width='30%'><h3>Status</h3></td><td width='30%'><h3>Location</h3></td><td width='10%'><h3>Control</h3></td></tr>"

	for (Bot in GLOB.alive_mob_list)
		if(Bot.get_z_level(TRUE) == ai_Zlevel && !Bot.remote_disabled) //Only non-emagged bots on the same Z-level are detected!
...
/mob/living/silicon/ai/triggerAlarm(class, area/A, O, obj/alarmsource)
	if(alarmsource.get_z_level(TRUE) != get_z_level(TRUE))
		return
